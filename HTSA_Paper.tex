...
\begin{piterpara}
This balanced design is important for the rest of the paper because the same 20-sample subset supports both the main side-specific comparisons and the secondary genotype interpretation. In other words, the workflow did not switch datasets when the later follow-up sets were defined; the 709-gene WT branch and the 870-gene cKO branch both came from the same retained DRG cohort and the same model-fitting framework \parencite{halawani2023ahr,sraStudySRP618841,bioprojectPRJNA1322439,geoGSE243308}.
\end{piterpara}

\begin{table}[H]
\centering
\caption{Summary of the 20-sample mouse DRG RNA-seq subset used for reanalysis.}
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{1.55in}|>{\raggedright\arraybackslash}p{1.05in}|>{\raggedright\arraybackslash}p{0.75in}|>{\centering\arraybackslash}p{0.45in}|Y|Y|}
\hline
\rowcolor{softgray}\textbf{Group} & \textbf{Side class} & \textbf{Genotype} & \textbf{n} & \textbf{Tissue / time point} & \textbf{Source dataset and platform} \\
\hline
WT contralateral & contralateral & WT & 5 & L4--L6 DRG, 1 day post-injury & \artifact{SRP618841} / \artifact{PRJNA1322439} / \artifact{GSE243308}; paired-end Illumina NovaSeq X \\
\hline
WT ipsilateral & ipsilateral & WT & 5 & L4--L6 DRG, 1 day post-injury & \artifact{SRP618841} / \artifact{PRJNA1322439} / \artifact{GSE243308}; paired-end Illumina NovaSeq X \\
\hline
cKO contralateral & contralateral & cKO & 5 & L4--L6 DRG, 1 day post-injury & \artifact{SRP618841} / \artifact{PRJNA1322439} / \artifact{GSE243308}; paired-end Illumina NovaSeq X \\
\hline
cKO ipsilateral & ipsilateral & cKO & 5 & L4--L6 DRG, 1 day post-injury & \artifact{SRP618841} / \artifact{PRJNA1322439} / \artifact{GSE243308}; paired-end Illumina NovaSeq X \\
\hline
\end{tabularx}
...
\begin{piterpara}
\textbf{Workflow reproducibility and version tracking.} To keep the paper reproducible at a grad-level standard, the main software tools and reference resources used in the local mouse RNA-seq workflow were tracked alongside the command-level examples. The draft explicitly records \tool{fastp} v0.23.2 and the GRCm39 / Ensembl release 115 reference pair, and the remaining tools below summarize the same implementation path used across download, QC, alignment, differential expression, and enrichment interpretation \parencite{sratoolkit,fastqc,multiqc,fastp,star,ensemblRelease115,deseq2,gprofiler2021}.
\end{piterpara}

\begin{table}[H]
\centering
\caption{Software, reference resources, and releases used in the mouse RNA-seq workflow.}
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{1.05in}|>{\raggedright\arraybackslash}p{2.05in}|Y|}
\hline
\rowcolor{softgray}\textbf{Stage} & \textbf{Tool / resource} & \textbf{Version, release, or implementation used} \\
\hline
Collection & SRA Toolkit (\tool{prefetch}, \tool{fasterq-dump}) & SRA Toolkit workflow used for \artifact{SRP618841} download and FASTQ generation \\
\hline
Cleaning & \tool{FastQC} & FastQC workflow used for raw and post-trim quality control \\
\hline
Cleaning & \tool{MultiQC} & MultiQC workflow used for cohort-level QC summaries \\
\hline
Cleaning & \tool{fastp} & v0.23.2 \\
\hline
Preparation & \tool{STAR} & STAR workflow used for paired-end alignment and \tool{GeneCounts} output \\
\hline
Preparation & Ensembl mouse reference genome & \textit{Mus musculus} GRCm39 primary assembly \\
\hline
Preparation & Ensembl gene annotation & Release 115 \\
\hline
Analysis & R / \tool{DESeq2} & R-based DESeq2 workflow used for count-based differential expression analysis \\
\hline
Analysis & Python & Python 3 workflow used for bend-point follow-up scripting and output consolidation \\
\hline
Analysis & \tool{g:Profiler} & 2021 update web service used for functional enrichment follow-up \\
\hline
\end{tabularx}
\renewcommand{\arraystretch}{1.0}
\end{table}
...
\begin{piterpara}
Downstream interpretation followed a PCA-first, branch-priority logic. PCA was used to evaluate sample structure before emphasizing long differential-expression tables, and the two side-specific branches (\artifact{ipsi\_vs\_contra\_in\_ff} and \artifact{ipsi\_vs\_contra\_in\_cre}) were then carried forward as the primary follow-up path because they matched the strongest sample-structure signal. To avoid reducing very large DE outputs to unstructured gene lists, the workflow applied the same bend-point rule to the ordered adjusted-\textit{p}-value curve for each contrast, marking the transition between the steep high-confidence portion of the ranked list and the flatter long tail. The resulting follow-up sets were then submitted to \tool{g:Profiler} using GO Biological Process, KEGG, and Reactome layers \parencite{gprofiler2021,geneOntology2021,kegg2025,reactome2022}.
\end{piterpara}
...
\begin{piterpara}
The same narrowing rule was then applied to the \artifact{ipsi\_vs\_contra\_in\_cre} branch, which began with 7{,}541 significant genes and converged to an 870-gene follow-up set at a branch-specific bend point of \(8.40 \times 10^{-17}\). This second branch did not come from a different dataset or a separate ad hoc analysis; it came from the same fitted DESeq2 model and the same ordered adjusted-\textit{p}-value procedure used for the WT branch. That parallel treatment is important because it shows that the side-specific injury signal remains strong in cKO as well, while also leaving room to compare how genotype background expands or shifts the follow-up set \parencite{deseq2}.
\end{piterpara}
